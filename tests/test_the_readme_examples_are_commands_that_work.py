...
one kind quietly is how a page keeps a command that no longer exists:
  ...
  agentlog save -m "before agent refactor"     an example: it has to parse
  agentlog save [-m MSG] [--force]             a synopsis: `[` and `<` mean the
...
from agentlog.cli import _build_parser as build_parser  # noqa: E402
...
def just_the_command(line):
    ...
    separated by a run of spaces:
        ...
        agentlog where                             print the snapshot directory
...
def as_a_shell_would_split_it(command):
    ...
    Stops at the first shell operator: `agentlog diff | less` is one command
...
class TestTheReadmeExamplesAreCommandsThatWork(unittest.TestCase):
    ...
    def test_they_are_commands_and_not_just_the_bare_tool(self):
        # `agentlog` on its own parses under any parser that has ever existed, so
